...
namespace Wraith
{
    ...
    void Renderer::CreateCommandBuffers()
    {
        for (auto& frame : m_Frames)
        {
            ...
            WR_VK_CHECK_MSG(vkAllocateCommandBuffers(m_Device->GetVkDevice(), &allocInfo, &frame.CommandBuffer),
    ...
    void Renderer::CreateRenderPass()
    {
        ...
        WR_VK_CHECK_MSG(vkCreateRenderPass(m_Device->GetVkDevice(), &renderPassInfo, nullptr, &m_RenderPass),
    ...
    void Renderer::CreateFramebuffers()
    {
        ...
        for (size_t i = 0; i < imageViews.size(); i++)
        {
            ...
            VkFramebufferCreateInfo framebufferInfo = VkFactory::FramebufferCreateInfo(
                m_RenderPass, attachments, swapChainExtent);
            WR_VK_CHECK_MSG(vkCreateFramebuffer(m_Device->GetVkDevice(), &framebufferInfo, nullptr, &m_Framebuffers[i]),
    ...
    void Renderer::CreateSyncStructures()
    {
        ...
        for (auto& frame : m_Frames)
        {
            WR_VK_CHECK_MSG(vkCreateFence(m_Device->GetVkDevice(), &fenceInfo, nullptr, &frame.RenderFence),
            ...
            WR_VK_CHECK_MSG(vkCreateSemaphore(m_Device->GetVkDevice(), &semaphoreInfo, nullptr, &frame.RenderSemaphore),
                            "Failed to create Render semaphore!")
            WR_VK_CHECK_MSG(
                vkCreateSemaphore(m_Device->GetVkDevice(), &semaphoreInfo, nullptr, &frame.PresentSemaphore),
    ...
    VkCommandBuffer Renderer::BeginFrame()
    {
        ...
        WR_VK_CHECK_MSG(vkWaitForFences(m_Device->GetVkDevice(), 1, &frame.RenderFence, true, 1000000000),
                        "Unable to wait for Render fence!")
        WR_VK_CHECK_MSG(vkResetFences(m_Device->GetVkDevice(), 1, &frame.RenderFence), "Unable to reset Render fence!")

        const VkResult result = m_Swapchain.
            AcquireNextImage(&m_SwapchainImageIndex, 1000000000, frame.PresentSemaphore);
    ...
    {
        WR_ASSERT_MSG(commandBuffer == GetCurrentFrame().CommandBuffer,
    ...
    void Renderer::SubmitCommandQueue()
    {
        ...
        WR_VK_CHECK_MSG(vkQueueSubmit(m_Device->GetGraphicsQueue(), 1, &submitInfo, frame.RenderFence),
                        "Unable to submit command queue!")
    }
}
